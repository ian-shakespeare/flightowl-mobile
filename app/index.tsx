      ...
      {focusedInput ? (
        <StyledScrollView className="h-full">
          ...
          {fuse
            ...
            .slice(0, 15)
            .map((pa, i) => (
              <StyledView key={i} className="py-2 border-b-2 border-gray-200">
                <ButtonSecondary
                  label={pa.item.name}
                  onPress={() => {
                    updateSearchParam(focusedInput, pa.item.iata_code);
        ...
        <StyledView className="h-2/3 flex gap-4 items-center justify-center">
          ...
          <StyledView className="w-3/4 py-2 px-4 flex flex-row justify-between items-center border-2 border-gray-200 rounded-md">
            ...
            {destination !== "" && (
              <IconButton
                ...
                color="#e5e7eb"
                size={24}
                onPress={() => updateSearchParam("destination", "")}
              />
            )}
          </StyledView>
          <StyledTextInput
          ...
          {origin !== "" && destination !== "" && date !== "" && (
            <ButtonPrimary
              ...
              onPress={() => console.log(origin, destination, date)}
            />
          )}
        </StyledView>
      )}
